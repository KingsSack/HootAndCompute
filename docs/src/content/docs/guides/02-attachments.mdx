...
An `Attachment` represents a distinct subsystem of your [Robots](../01-robots), such as an intake, a claw, or an arm. The `Attachment` base class provides a powerful structure for managing state, creating [Actions](../../concepts/01-actions), and displaying telemetry, all of which are automatically handled by the `Robot` class.

:::note

[Attachments](../02-attachments) can be shared by multiple [Robots](../01-robots).

:::

## Steps

There should be one `Attachment` per physical attachment/subsystem your team has built.
Ideally, it operates independently from your other subsystems.

:::caution[Important]

This page goes over creating your own `Attachment` from the abstract base class, which isn't always necessary.

See [Using Built-In Attachments](../03-built-in-attachments) to learn more.

:::

:::tip[Prerequisites]

This guide assumes you've [created a Robot class](../01-robots) and understand 
...
First, create a new class that inherits `Attachment`. The `Attachment` constructor
requires a `name` for the attachment, which is used as an identifier and for 
logging telemetry. This `name` should be unique for each `Attachment` you create.

This class will need to receive a reference to all the hardware it will control.
 Examples of other hardware include:
...
The `Robot` class automatically calls the `update()` method on every registered `Attachment` during each tick of your [OpMode](../05-opmodes).

The base `Attachment` already implements `update()`  to add telemetry about the `Attachment`'s state
(`Idle`, `Running`, or `Fault`). If you need continuous logic or custom telemetry, you can override this method.
...
Every `Attachment` has a state, which is managed automatically when using [Actions](../../concepts/01-actions). The possible states are:
...
In this example, when the [Actions](../../concepts/01-actions) built by either `open()` or `close()` is executed,
...
Using the `attachment` builder method, the `Robot` will store it and automatically
handle the `Attachment`'s `update()` method.
...
- [Create an OpMode](../05-opmodes) that controls your attachment
- [Discover Built-In Attachments](../03-built-in-attachments)
- [Discover Drivetrains](../04-drivetrains)
